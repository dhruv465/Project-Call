...
const CampaignForm = ({ campaignId, onClose, onSuccess }: CampaignFormProps) => {
    ...
    try {
      ...
      setTimeout(() => {
        ...
        setFormData({
          ...
          // ... other fields would be populated from API response
        });
        setIsLoading(false);
      }, 500);
    } catch (error) {
  ...
  // Load data when the component mounts if editing
  useState(() => {
    if (campaignId) {
      loadCampaignData();
    }
              ...
              {currentTab === 'script' && (
                <div className="space-y-4">
                  ...
                  <div className="bg-muted/50 p-4 rounded-md">
                    <h4 className="text-sm font-medium mb-2">Script Guidelines</h4>
                    <ul className="text-sm space-y-1 list-disc pl-5">
                      <li>Keep the script conversational and natural-sounding</li>
                      <li>Include alternative paths for different responses</li>
                      <li>Keep introductions brief and clear</li>
                      <li>Focus on value proposition early in the call</li>
                      <li>Include clear call-to-action or next steps</li>
                    </ul>
                  </div>
                </div>
              )}
                  ...
                  <div>
                    ...
                    <div className="bg-muted/50 p-3 rounded-md">
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                        {['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'].map((day) => (                              <label key={day} className="flex items-center gap-2">
                                <input
                                  type="checkbox"
