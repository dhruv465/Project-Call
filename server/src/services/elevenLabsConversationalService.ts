...
import { EmotionAnalysis, VoicePersonality } from './voiceAIService';
  ...
  public async synthesizeAdaptiveVoice(params: {
    ...
    try {
      ...
      const audioBuffer = await this.generateSpeech(
        ...
        personalityId,
        {
          stability: adaptiveSettings?.stability,
          similarityBoost: adaptiveSettings?.similarityBoost,
          style: adaptiveSettings?.style,
          modelId
        }
      );
